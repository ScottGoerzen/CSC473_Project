...
void main()
{
    fragColour = colour;
            ...
            mNumIndices(100)
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                vertices[4 * i + 0] = { -100.0f + i, -0.15f, -50.0f };
                vertices[4 * i + 1] = { -100.0f + i, -0.15f,  50.0f };
                vertices[4 * i + 2] = { -100.0f, -0.15f, -50.0f + i };
                vertices[4 * i + 3] = {  100.0f, -0.15f, -50.0f + i };
            }
            ...
            math::Matrix4 const& view)
        {
            ...
            glDrawArrays(GL_TRIANGLES, n, 4);

            glPointSize(5.0f);
            float rgb = (64.0f / 255.0f);
            glUniform4f(mUniforms["colour"], rgb, rgb, rgb, 1);
            glDrawArrays(GL_TRIANGLES, 0, (int)mNumIndices);
